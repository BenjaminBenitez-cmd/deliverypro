...
import { Card, CardBody, CardHeader, CardTitle, ModalBody, ModalFooter, Table, Button, Modal, Row, Col, CustomInput } from 'reactstrap'
...
const DeliveryTable = () => {
    ...
    return (
        <>
            <Card>
              ...
              <CardBody>
                <Table className="tablesorter" responsive>
                  ...
                  <tbody>
                      {
                         deliveries && deliveries.map((node, index) => (
                              <tr key={node.id} onClick={() => handleToggleOpen(node.id)}>
                                  <td>
                                    <CustomInput type="switch" id={`switch-${node.id}`} onClick={() => dispatch(toggleDelivery(node.id))}/>
                                  </td>
            ...
            { isOpen && <DeliveryModal isOpen={isOpen} toggleModal={handleToggleOpen} information={delivery} /> }
          </>
    )
}
  ...
  return (
    ...
    <Modal isOpen={isOpen} toggle={toggleModal}>
      ...
      <ModalBody>
          <Row>
            <Col sm={6}>
              <div className="py-2"><strong>Buyer</strong> {information.first_name + ' ' + information.last_name}</div>
              <div className="py-2"><strong>Email</strong> {information.email}</div>
              <div className="py-2"><strong>Phone Number</strong> {information.phone_number}</div>
              <div className="py-2"><strong>Delivery On</strong> {information.delivery_date}</div>
              <div className="py-2"><strong>Location</strong> {information.street + ', ' + information.district }</div>
            </Col>
          </Row>
      </ModalBody>
    ...
    {isEditModalOpen && <DeliveryEditModal isOpen={isEditModalOpen} toggleModal={handleToggle} information={{hello: 'hello'}}/> }
    </>
  )
}
...
const DeliveryEditModal = ({ isOpen, toggleModal, information }) => (
  <>
    <Formik
      initialValues={{ phone_number: '', street: '', district: '', delivery_date: '', delivery_time: ''}}
